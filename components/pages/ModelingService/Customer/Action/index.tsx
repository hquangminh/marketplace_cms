...
import { CustomerActionProps, ParamsType } from 'models/customer.model';

import styled from 'styled-components';

const CustomerActionModal = (props: CustomerActionProps) => {
  const {
    modalLists,
    setModalLists,
    loading,
    onFetchEditCustomer,
    onFetchCreateCustomer,
    onFetchDeleteCustomer,
    form,
    fileList,
    setFileList,
  } = props;
  ...
  const onSubmit = (values: ParamsType) => {
    ...
    if (params['image'] !== modalLists.data?.image) {
      params.oldImage = modalLists.data?.image || '';
      params.image = fileList[0]?.image || '';
    }

    if (modalLists.type === 'edit') {
      onFetchEditCustomer(modalLists.data?.id as string, params);
      return;
    }
    onFetchCreateCustomer(params);
  };
  ...
  const deleteModal = () => {
    showConfirmDelete(
      ...
      `Order Id: ${modalLists.data?.orderid}`
    );
  };

  return (
    <Modal
      title={modalLists.type ? capitalizeFirstLetter(modalLists.type) + ' customer' : ''}
      centered
