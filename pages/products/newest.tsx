...
const Index = (props: PropsPageType) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await productServices.getProductNewest((page - 1) * pageSize, pageSize, name);
  ...
  useEffect(() => {
    fetchData();
  }, [page, productLists.total, name]);

  return (
    <Fragment>
      <Head>
        <title>Market Place Admin - Product Newest</title>
      </Head>

      <>
        <HeaderPageFragment
          title='List Product Newest'
...
export default withAuth(
  withLayout(Index, {
    sidebar: { openKeys: ['products'], selectedKey: 'products-newest' },
  })
);
