...
const Index = (props: PropsPageType) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await productServices.getProductHotest((page - 1) * pageSize, pageSize, name);
      if (!res.error)
        setProductLists({
          total: res.total,
          data: res.data,
  ...
  return (
    <Fragment>
      <Head>
        <title>Market Place Admin - Product Hotest</title>
      </Head>

      <>
        <HeaderPageFragment
          title='List Product Hotest'
...
export default withAuth(
  withLayout(Index, {
    sidebar: { openKeys: ['products'], selectedKey: 'products-hotest' },
  })
);
